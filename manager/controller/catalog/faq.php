...
class ControllerCatalogFaq extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_catalog_information->addFaq($this->request->post);
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->model_catalog_information->editFaq($this->request->get['faq_id'], $this->request->post);
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $faq_id) {
				$this->model_catalog_information->deleteFaq($faq_id);
			}
	...
	protected function getList() {
		...
		$data['enabled'] = $this->url->link('catalog/faq/enable', 'user_token=' . $this->session->data['user_token'] . $url, true);
		$data['disabled'] = $this->url->link('catalog/faq/disable', 'user_token=' . $this->session->data['user_token'] . $url, true);
		$data['ajax_pub'] = $this->url->link('catalog/faq/set_published', 'user_token=' . $this->session->data['user_token'] . $url, true);
		...
		$information_total = $this->model_catalog_information->getTotalFaqes();

		$results = $this->model_catalog_information->getFaqes($filter_data);

		foreach ($results as $result) {
			$data['informations'][] = array(
				'faq_id' => $result['id'],
				'gbname'          => $result['gbname'],
				'gbmail'     => $result['gbmail'],
				'gbloca'  	  	 => $result['gbloca'],
				'gbtext'  	  	 => $this->preview_text($result['gbtext'], 50),
				'gbtextfull'  	  	 => $result['gbtext'],
				'gbcommentfull'  	  	 => $result['gbcomment'],
				'gbcomment'  	  	 => $this->preview_text($result['gbcomment'], 50),
				'gbdate'  	  	 => $result['gbdate'],
				'published'  	  	 => $result['published'],
				'edit'           => $this->url->link('catalog/faq/edit', 'user_token=' . $this->session->data['user_token'] . '&faq_id=' . $result['id'] . $url, true)
			);
		}
		...
		$data['sort_title'] = $this->url->link('catalog/faq', 'user_token=' . $this->session->data['user_token'] . '&sort=gbdate' . $url, true);
		$data['sort_sort_order'] = $this->url->link('catalog/faq', 'user_token=' . $this->session->data['user_token'] . '&sort=i.sort_order' . $url, true);
		$data['sort_noindex'] = $this->url->link('catalog/faq', 'user_token=' . $this->session->data['user_token'] . '&sort=i.noindex' . $url, true);
	...
	protected function getForm() {
		...
		if (isset($this->error['gbtext'])) {
			$data['error_gbtext'] = $this->error['gbtext'];
		} else {
			$data['error_gbtext'] = array();
		}

		if (isset($this->error['gbname'])) {
			$data['error_gbname'] = $this->error['gbname'];
		} else {
			$data['error_gbname'] = array();
		}
		...
		if (isset($this->request->get['faq_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$information_info = $this->model_catalog_information->getFaq($this->request->get['faq_id']);
		}
		...
		if (isset($this->request->post['information_description'])) {
			...
			//$data['information_description'] = $this->model_catalog_information->getInformationDescriptions($this->request->get['faq_id']);
			$data['information_description'][1] = $information_info;
		} else {
		...
		if (isset($this->request->post['information_store'])) {
			...
			$data['information_store'] = $this->model_catalog_information->getInformationStores($this->request->get['faq_id']);
		} else {
		...
		if (isset($this->request->post['information_seo_url'])) {
			...
			$data['information_seo_url'] = $this->model_catalog_information->getInformationSeoUrls($this->request->get['faq_id']);
		} else {
		...
		if (isset($this->request->post['information_layout'])) {
			...
			$data['information_layout'] = $this->model_catalog_information->getInformationLayouts($this->request->get['faq_id']);
		} else {
	...
	protected function validateForm() {
		...
		foreach ($this->request->post['information_description'] as $language_id => $value) {
			if ((utf8_strlen($value['gbname']) < 1) || (utf8_strlen($value['gbname']) > 64)) {
				$this->error['gbname'][$language_id] = $this->language->get('error_title');
			}

			if (utf8_strlen($value['gbtext']) < 3) {
				$this->error['gbtext'][$language_id] = 'Введите вопрос';
			}
		...
		// 					if (($seo_url['store_id'] == $store_id) && (!isset($this->request->get['faq_id']) || ($seo_url['query'] != 'faq_id=' . $this->request->get['faq_id']))) {
	...
	public function enable() {
        ...
        if (isset($this->request->post['selected']) && $this->validateEnable()) {
            foreach ($this->request->post['selected'] as $faq_id) {
                $this->model_catalog_information->editInformationStatus($faq_id, 1);
            }
            $this->session->data['success'] = $this->language->get('text_success');
            $url = '';
            if (isset($this->request->get['page'])) {
    ...
    public function disable() {
        ...
        if (isset($this->request->post['selected']) && $this->validateDisable()) {
            foreach ($this->request->post['selected'] as $faq_id) {
                $this->model_catalog_information->editInformationStatus($faq_id, 0);
            }
            $this->session->data['success'] = $this->language->get('text_success');
            $url = '';
            if (isset($this->request->get['page'])) {
	...
	public function set_published() {
		...
		$json = [];
		if(empty($this->request->post['faq_id'])){ return false; }

		if(empty($json['error'])){

			$this->load->model('catalog/information');
			$this->model_catalog_information->set_published($this->request->post);
