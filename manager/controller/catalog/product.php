...
class ControllerCatalogProduct extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->get['filter_new'])) {
				$url .= '&filter_new=' . $this->request->get['filter_new'];
			}			
	...
	protected function getList() {
		...
		if (isset($this->request->get['filter_new'])) {
			$filter_quantity = $this->request->get['filter_new'];
		} else {
			$filter_quantity = '';
		}		
		if (isset($this->request->get['filter_quantity_min'])) {
			...
			if (isset($product_option['product_option_value'])) {
				foreach ($product_option['product_option_value'] as &$product_option_value) {
					...
					if(!empty($product_option_value['image']) && is_file(DIR_IMAGE . $product_option_value['image'])) {
						$product_option_value['thumb'] = $this->model_tool_image->resize($product_option_value['image'], 100, 100);
					} else {
						$product_option_value['thumb'] = $this->model_tool_image->resize('no_image.png', 100, 100);
					}
