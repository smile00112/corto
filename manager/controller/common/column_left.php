	...
	public function index() {
		if (isset($this->request->get['user_token']) && isset($this->session->data['user_token']) && ($this->request->get['user_token'] == $this->session->data['user_token'])) {
			...
			if ($catalog) {
				...
				if($this->user->getGroupId() == 2){//Для менеджера вовыдим только список ммагазинов
					$data['menus'][] = array(
						'id'       => 'menu-blog',
						'icon'	   => 'fa-university', 
						'name'	   => 'МАГАЗИНЫ CORTO',
						'href'     => $this->url->link('catalog/information/edit', 'user_token=' . $this->session->data['user_token'].'&information_id=8', true),
