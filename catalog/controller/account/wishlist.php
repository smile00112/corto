		...
		foreach ($results as $result) {
			...
			if ($product_info) {
				...
				$data['products'][] = array(
					...
					'price'      => $price,
					'price_not_formated' => number_format( $this->tax->calculate($product_info['price'], $product_info['tax_class_id'], $this->config->get('config_tax')), 0, '.', ' '),
					'special'    => $special,
	...
	public function remove() {
		...
		if ($product_info) {
			if ($this->customer->isLogged()) {
				...
				if(($delete_key = array_search($this->request->post['product_id'], $this->session->data['wishlist'])) !== false)
				{
					unset($this->session->data['wishlist'][$delete_key]);
				}
