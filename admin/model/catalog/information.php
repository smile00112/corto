...
class ModelCatalogInformation extends Model {
	...
	public function set_published($data) {
		$this->db->query("UPDATE " . DB_PREFIX . "faq  SET published = '".(int)$data['faq_pub']."' WHERE id='".(int)$data['faq_id']."'");
		return false;
	}
	
	public function getTotalFaqes() {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "faq");

		return $query->row['total'];
	}
	public function getFaq($faqId) {
		$query = $this->db->query("SELECT *  FROM " . DB_PREFIX . "faq WHERE id='".(int)$faqId."'");
	...
	public function addFaq($data = array()) {
		...
		$query = $this->db->query("INSERT INTO " . DB_PREFIX . "faq SET
			`gbname` = '".$this->db->escape($data['gbname'])."',
			`gbmail` = '".$this->db->escape($data['gbmail'])."',
			`gbloca` = '".$this->db->escape($data['gbloca'])."',
			`gbtext` = '".$this->db->escape($data['gbtext'])."',
			`gbcomment` = '".$this->db->escape($data['gbcomment'])."',
	...
	public function editFaq($faq_id, $data = array()) {
		$data = $data['information_description'][1];

		if(!isset($data['published'])) $data['published'] = 0;
		$query = $this->db->query("UPDATE " . DB_PREFIX . "faq SET
			`gbname` = '".$this->db->escape($data['gbname'])."',
			`gbmail` = '".$this->db->escape($data['gbmail'])."',
			`gbloca` = '".$this->db->escape($data['gbloca'])."',
			`gbtext` = '".$this->db->escape($data['gbtext'])."',
	...
	public function deleteFaq($faqId) {
		$query = $this->db->query("DELETE FROM  " . DB_PREFIX . "faq  WHERE id='".(int)$faqId."'");

		return;
	}
	public function getFaqes($data = array()) {
		if ($data) {
			$sql = "SELECT * FROM " . DB_PREFIX . "faq i  WHERE 1";
